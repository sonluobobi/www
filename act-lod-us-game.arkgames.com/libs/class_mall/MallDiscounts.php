...
class MallDiscounts {
	...
	var $DB = null; 							//数据库句柄
	var $tbl_mall_discount = 'tbl_mall_discount';    //折扣表名

	//表字段
	var $tbl_mall_discount_fields = array ( 'id', 'activity_sort_id', 'title', 'character_level', 'interfix_npc_id', 'interfix_npc', 'start_time', 'end_time', 'server_ids', 'intro', 'is_view_act', 'published', 'created', 'updated' );
	...
	function getActCount($condition = '') {
		$sql = "SELECT COUNT(`id`) AS total " . " FROM " . $this->tbl_mall_discount . " WHERE published=1 " . $condition;
	 ...
	 * 
	 */
	function getMallDiscountList($condition = '', $order_by = '', $limit = '', $offset = '') {
		$str_fields = $this->formatFields ( $this->tbl_mall_discount_fields );
		
		$order_byStr = $order_by == '' ? " ORDER BY id DESC " : " ORDER BY " . $order_by . " DESC ";
		$limitStr = $limit == '' ? '' : " LIMIT " . $limit;
		$offsetStr = $offset == '' ? '' : " OFFSET " . $offset;
		
		$sql = "SELECT * FROM " . $this->tbl_mall_discount ." WHERE 1 " . 
...
//		die($sql);
	 ...
	 * 
	 */
	function addMallDiscount($arrData) {
		$sql = "INSERT INTO " . $this->tbl_mall_discount . "(" . $arrData ['fields'] . ") VALUES(" . $arrData ['values'] . ")";
...
//		$this->update2memcache();
	 ...
	 * 
	 */
	function getMallDiscountById($id) {
		$str_fields = $this->formatFields ( $this->tbl_mall_discount_fields );
		
		$sql = "SELECT * FROM " . $this->tbl_mall_discount . " WHERE  `id` = " . $id . " LIMIT 1";
	 ...
	 * 
	 */
	function updateMallDiscount($id, $strData) {
		$sql = "UPDATE " . $this->tbl_mall_discount . " SET " . $strData . " WHERE id = " . $id . " LIMIT 1";
...
//		$this->update2memcache();
	...
	function deleteDiscountItemById($id) {
		$sql = "DELETE FROM " . $this->tbl_mall_discount . " WHERE id = " . $id . " LIMIT 1";
