...
<script type="text/javascript">
	...
	function showLuaError(filename, id)
	{
		...
		if(display_status == 'none')
		{
			...
			var pars = 'do_search=search&server_id='+server_id+'&content_id='+id+'&filename='+filename+'&r=' + Math.random();

			$.ajax({
				url:"luaErrorPage.php",
				async:false,
				data:pars,
				type:"POST",
				beforeSend:function(){
	...
	function showLuaErrorMore(id, more_div_id, filename, end_line)
	{
		var content_id='#'+more_div_id;
		$(content_id).html("查询中...");

		var pars = 'do_search=search&server_id='+server_id+'&content_id='+id+'&end_line='+end_line+'&filename='+filename+'&r=' + Math.random();
