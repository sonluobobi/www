<?php
	...
	else
	{
		...
		$check_sign = 'pl';
		$name = '服务器列表加载域名';
		$url = 'https://'. $check_sign . $https_base_domain . '/t.php';
		...
		$check_sign = 'res';
		$name = '测试版资源加载域名';
		$url = 'https://'. $check_sign . $https_base_domain . '/config.txt?t='.time();
		$result = do_get_checkhttps($url);
		...
		if (!empty($server_list))
		{
			...
			{
				$url = 'https://'. $sx . $https_base_domain . '/t.php';
	...
	$smarty->assign('caption', 'https连接检查');
