	...
	public function sendPropsEquip()
	{
		if($_POST)
		{
			if(isset($_POST['chargecancel_wenjuan_id']))
			{
				...
				$newfile = ".question.php";

				//$newfile = "/data/syslog/question.php";
				/*if(!is_dir($text)){
                    mkdir(iconv("UTF-8", "GBK", $text),0777,true);
                }*/

				if (file_exists($newfile)) {
					$return  = file_put_contents($newfile, '');
					common\Functions::alertFunc('设置成功2=清空文件' );
				}
				else {
					$fh = fopen($newfile, 'wb');
					...
					common\Functions::alertFunc('设置成功2=创建文件' );
				}
			}
			else
			{
